...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        if sampling_error==0:
            ...
            self.attn = MultiheadAttentionSampling(
                embed_dims=embed_dims,
                num_heads=num_heads,
                attn_drop=attn_drop_rate,
                proj_drop=drop_rate,
...
class FFNSamplingDefer(BaseModule):
    ...
    def forward(self, x, identity=None):
        ...
        if identity is None:
            identity = x
        return self.sampling(identity + self.dropout_layer(out))
...
@MODELS.register_module()
class VisionTransformerSamplingDefer(BaseBackbone):
    """Vision Transformer.
    ...
    arch_zoo = {
        ...
        **dict.fromkeys(
            ['cifar10', 'cifar-10'], {
                ...
                'num_layers': 8,
                'num_heads': 8,
                'feedforward_channels': 48 * 3,
            }),
        **dict.fromkeys(
            ['cifar10-ss', 'cifar-10-ss'], {
                'embed_dims': 12*8,
                'num_layers': 8,
                'num_heads': 8,
                'feedforward_channels': 12*8*2,
            }),
        **dict.fromkeys(
            ['cifar10-s', 'cifar-10-s'], {
                'embed_dims': 12*16,
                'num_layers': 16,
                'num_heads': 16,
                'feedforward_channels': 12*16*2,
            }),
        **dict.fromkeys(
            ['cifar10-b', 'cifar-10-b'], {
                'embed_dims': 48*8,
                'num_layers': 8,
                'num_heads': 8,
                'feedforward_channels': 48 * 3,
    ...
    def __init__(self,
        ...
        super(VisionTransformerSamplingDefer, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(VisionTransformerSamplingDefer, self).init_weights()
